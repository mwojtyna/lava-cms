...
const PAGE: Parameters<typeof privateCaller.pages.addPage>[0] = {
	name: "Page",
	url: "/page",
...
it("adds a page and returns its id", async () => {
	...
	await expect(privateCaller.pages.addPage(PAGE)).resolves.toBe(ID);
...
it("throws a trpc 409 'CONFLICT' error if the page url already exists", async () => {
	...
	await expect(privateCaller.pages.addPage(PAGE)).rejects.toThrowError(
